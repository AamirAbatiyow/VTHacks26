  ...
  return (
    <div className="progress-garden">
      ...
      <div className="progress-garden__content">
        ...
        <footer className="progress-garden__footer"><span aria-hidden="true">◌</span><p>{saved ? "Session time and counts are saved on this device for your name. Audio and transcripts aren’t saved in your progress history." : "Device storage is unavailable. Your progress is available for this visit."}</p></footer>
      </div>
      <SpeechAnalytics profile={profile} sessions={sessions} reportOnly reportOpen={reportOpen} onReportClose={() => setReportOpen(false)} />
    </div>
  );
}
